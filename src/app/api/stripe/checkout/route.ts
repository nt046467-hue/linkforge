  ...
  try {
    ...
    if (!stripeSecretKey || stripeSecretKey === 'sk_test_placeholder') {
      ...
      return NextResponse.json({
        url: null,
        demoMode: true,
        message: 'Demo mode — Pro plan activated! Add STRIPE_SECRET_KEY to .env for real payments.',
      })
    }
    ...
    if (!priceId) {
      return NextResponse.json({
        error: 'Stripe price not configured. Add STRIPE_MONTHLY_PRICE_ID and STRIPE_YEARLY_PRICE_ID to .env',
      }, { status: 500 })
    }
