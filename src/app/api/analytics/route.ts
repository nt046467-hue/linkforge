...
export async function GET(req: NextRequest) {
  try {
    ...
    const dailyViews = await db.$queryRaw<Array<{ date: string; views: bigint }>>`
    ...
    const dailyViewsMap = new Map(dailyViews.map((d) => [d.date, Number(d.views)]))
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      filledDailyViews.push({
        date: dateStr,
        views: Number(dailyViewsMap.get(dateStr) ?? 0),
      })
    }
